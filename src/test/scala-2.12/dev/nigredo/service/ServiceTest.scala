...
class ServiceTest extends FunSuite with ScalaFutures with Matchers {
  ...
  test("try update non existing entity") {
    update[StrId, Dto, ExistingEntity, UpdateEntity](_ => None.fs)(_ => _ => updated)(_ => \/-(updated).fs)(_ => successOnUpdate)(id)(dto).futureValue.map(_ shouldEqual ItemNotFound())
  }

  test("update existing user with invalid data") {
    update[StrId, Dto, ExistingEntity, UpdateEntity](_ => Some(existing).fs)(_ => _ => updated)(_ => error)(_ => successOnUpdate)(id)(dto).futureValue.map(_ shouldEqual ValidationError(Nil))
  }

  test("update existing user with valid data") {
    update[StrId, Dto, ExistingEntity, UpdateEntity](_ => Some(existing).fs)(_ => _ => updated)(_ => \/-(updated).fs)(_ => successOnUpdate)(id)(dto).futureValue.map(_ shouldEqual updated)
  }
